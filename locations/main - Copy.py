...
def get_data(token,reducer):
    conn = http.client.HTTPSConnection("dataex.rimes.int")
    payload = json.dumps({
      "asset_identifier": "e18fc866-d139-4043-88ba-21847f6dfc26",
      "unique_field": "tehsil_id",
      "reducer": "tmin_daily_tmin_region"
    })
    headers = {
      'Authorization': token,
      'Content-Type': 'application/json'
...
def update_data_to_db(tehsil_id, date, tmin):
    try:
        conn = psycopg2.connect(
                dbname=connection_params["dbname"],
                user=connection_params["user"],
        ...
        cursor = conn.cursor()
        insert_query = """
        INSERT INTO public.temp_data (tehsil_id, tmin, date)
        VALUES (%s, %s, %s)
         """
...
def fetch_and_store_data():
    ...
    data_response = get_data(token,'tmax_daily_tmax_region')
...
@router.get("/dataex/fetch_and_store_data_tmin/")
def fetch_and_store_data_tmin():
    # Step 1: Authenticate and get the token
    auth_response = authenticate()
    ...
    # Step 2: Use the token to get the data
    data_response = get_data(token,'tmin_daily_tmin_region')
    data_dict = json.loads(data_response)
    ...
    for key in r_data:
        if 'time' in r_data[key] and 'value' in r_data[key]:
            ...
            for time_period, value in zip(r_data[key]['time'], r_data[key]['value']):
                ...
                # Insert data into the database
                update_data_to_db(tehsil_id, date, tmax)
        else:
            raise HTTPException(status_code=500, detail=f"No 'time' or 'value' field found for key: {key}")
